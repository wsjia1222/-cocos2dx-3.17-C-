...
bool StackView::init() {
    ...
    // 设置底牌堆的大小
    // STACK_WIDTH = 1080（屏幕宽度）
    // STACK_HEIGHT = 580（底牌堆高度）
...
void StackView::layoutCards() {
    ...
    float centerY = STACK_HEIGHT / 2;  // 底牌区垂直中心
    float cardWidth = 120.0f;  // 卡牌宽度（缩放后）
    float cardSpacing = 130.0f;  // 卡牌间距（未使用，但保留以备后用）
    
    // 主底牌位置（右边，距离右边缘200像素）
    Vec2 mainCardPos = Vec2(STACK_WIDTH - 200, centerY);
...
cocos2d::Vec2 StackView::getTopCardPosition() const {
    // 顶部卡牌位置在底牌区中心
    return cocos2d::Vec2(STACK_WIDTH / 2, STACK_HEIGHT / 2);
}
